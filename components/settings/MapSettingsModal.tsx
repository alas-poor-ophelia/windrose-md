...
function MapSettingsModalContent(): React.ReactElement | null {
  const settingsCtx = useMapSettings();
  const {
    isOpen,
    activeTab,
    setActiveTab,
  ...
  // dc.preact.render() creates a separate Preact tree, so context doesn't
  // cross the boundary. Bridge the MapSettingsContext into the portal tree.
  const bridgeContext = (children: unknown): React.ReactElement =>
    h(MapSettingsContext.Provider, { value: settingsCtx }, children);
