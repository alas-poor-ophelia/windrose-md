...
const { formatDistance } = await requireModuleByName("distanceOperations.ts") as {
  formatDistance: (distance: number, distancePerCell: number, distanceUnit: string) => string;
};
const { useMapSelection } = await requireModuleByName("MapSelectionContext.tsx") as {
  useMapSelection: () => {
    selectItem: (item: { type: string; id: string; data?: Record<string, unknown> } | null) => void;
    selectedItem: { type: string; id: string; data?: Record<string, unknown> } | null;
    setIsDraggingSelection: (value: boolean | ((prev: boolean) => boolean)) => void;
    setDragStart: (pos: { clientX: number; clientY: number; gridX?: number; gridY?: number; worldX?: number; worldY?: number } | null) => void;
  };
};
...
function useShapeOverlayTools(options: ShapeOverlayToolsOptions): UseShapeOverlayToolsResult {
  ...
  const [activeShape, setActiveShape] = dc.useState<ShapeOverlayType>('square');

  const [firstClick, setFirstClick] = dc.useState<Point | null>(null);
  const [currentWorldPos, setCurrentWorldPos] = dc.useState<Point | null>(null);
  ...
  const sizeToFormattedDistance = dc.useCallback((size: number): string => {
    ...
    return formatDistance(cellDistance, distancePerCell, distanceUnit);
  }, [geometry, mapData]);

  const computeShapeFromClicks = dc.useCallback((p1: Point, p2: Point): { center: Point; size: number } | null => {
  ...
  const preview = dc.useMemo((): ShapePreviewState | null => {
  ...
  const cancelPlacement = dc.useCallback(() => {
    setFirstClick(null);
    setCurrentWorldPos(null);
  }, []);

  const deleteShapeOverlay = dc.useCallback((id: string) => {
    if (!mapData) return;
    const shapes = (mapData.shapeOverlays || []).filter(s => s.id !== id);
    onShapeOverlaysChange(shapes);
  }, [mapData, onShapeOverlaysChange]);

  const updateShapeOverlay = dc.useCallback((id: string, updates: Partial<ShapeOverlay>) => {
    if (!mapData) return;
  ...
  const hitTestShape = dc.useCallback((worldX: number, worldY: number): ShapeOverlay | null => {
  ...
  const handleShapeSelection = dc.useCallback((clientX: number, clientY: number): boolean => {
    ...
    if (hit) {
      ...
      setDragStart({ clientX, clientY, worldX: world.worldX, worldY: world.worldY });
      return true;
    }
    return false;
  }, [screenToWorld, hitTestShape, selectItem, setIsDraggingSelection, setDragStart]);

  const dragPrevRef = dc.useRef<{ worldX: number; worldY: number } | null>(null);

  const handleShapeDragging = dc.useCallback((e: MouseEvent | TouchEvent) => {
  ...
  const stopShapeDragging = dc.useCallback(() => {
