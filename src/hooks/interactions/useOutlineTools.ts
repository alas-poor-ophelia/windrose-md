...
function useOutlineTools(options: OutlineToolsOptions): UseOutlineToolsResult {
  ...
  const [outlineSettings, setOutlineSettingsState] = useState<OutlineSettings>(DEFAULT_SETTINGS);
  ...
  const setOutlineSettings = useCallback((updates: Partial<OutlineSettings>) => {
  ...
  const handleContextMenu = useCallback((_e: MouseEvent) => {
  ...
  const deselectOutline = useCallback(() => {
    setSelectedOutlineId(null);
  }, []);
