...
import { getActiveLayer, initializeFogOfWar, fogCell, revealCell, fogRectangle } from '../../persistence/layerAccessor';
...
const useFogTools = (
  ...
  const [rectangleStart, setRectangleStart] = useState<FogRectangleStart | null>(null);
  const [rectangleHover, setRectangleHover] = useState<OffsetCoords | null>(null);
  const [lastCell, setLastCell] = useState<FogCellPosition | null>(null);
   ...
   * Uses geometry abstraction for coordinate conversion
   */
  const screenToOffset = useCallback(
   ...
   */
  const ensureFogInitialized = useCallback((): FogOfWar | null => {
   ...
   */
  const cancelFog = useCallback((): void => {
    setIsDrawing(false);
    setRectangleStart(null);
    setRectangleHover(null);
    setLastCell(null);
