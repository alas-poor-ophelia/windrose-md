...
export type GridDensityKey = 'sparse' | 'medium' | 'dense' | 'custom';

/** Sizing mode for background image */
export type SizingMode = 'density' | 'measurement';

/** Coordinate display mode */
export type CoordinateDisplayMode = 'rectangular' | 'radial' | 'axial' | 'none';

/** Color picker identifier */
export type ColorPickerId =
  | 'gridLineColor'
  | 'backgroundColor'
  | 'borderColor'
  | 'coordinateKeyColor'
...
/** Preference keys */
export type PreferenceKey = 'rememberPanZoom' | 'rememberSidebarState' | 'rememberExpandedState';
...
/** Image search result (currently just display name strings from imageOperations) */
export type ImageSearchResult = string;
...
/** Bounds shape mode */
export type BoundsShape = 'rectangular' | 'radial';
...
export interface BuildInitialStateProps {
...
interface ImageSelectedAction {
  ...
  payload: { path: string; displayName: string; dimensions: ImageDimensions; bounds: HexBounds | null };
}
