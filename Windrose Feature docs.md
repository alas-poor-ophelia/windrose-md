...
As of v1.6.1, Windrose uses `windrose-map` code blocks to embed maps in your notes. The "Insert new map" and "Generate random dungeon" commands create these automatically.
...
This replaces the older compiled-script embedding method. Existing maps continue to work.
...
- **Tile** tool group (**HEX ONLY**)
	- **Tile paint**: Paint tile images from a tileset onto hex cells. Click to place, drag to paint. Uses currently selected tile from the tile asset browser.
...
- **Note Pin**: Place a note pin on the map. Note pins link directly to vault notes and display as pin icons. Configurable color and icon.
...
- Change **Tileset folders** to scan for tile images (settings plugin)
...
It shows all of the objects in your current **object set**. A dropdown at the top of the sidebar lets you switch between available object sets without leaving the map. Clicking an object from the sidebar will automatically select the **object placement** tool, and clicking with it will place that object on the map. Current selection can be cleared with the **X** button at the bottom of the toolbar.
...
#### Tile Asset Browser
The **tile asset browser** opens when the tile paint tool is selected. It shows all available tilesets (configured in the settings plugin under "Tileset Folders"). Browse tiles by category (subfolders), and click to select a tile for painting.

#### Painting Tiles
With a tile selected, **click** to place it on a single hex, or **drag** to paint across multiple hexes. Tiles support:
- **Rotation** in 60-degree increments
- **Horizontal flip**
- **Base and overlay layers** — overlay tiles render on top of base tiles
- **Freeform stamps** — place tile images at arbitrary world-space positions, not snapped to the hex grid
