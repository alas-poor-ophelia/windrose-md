...
- [x] **13** Tablet `.is-tablet` spot-check — **DONE & live-verified (2026-06-26).** Obsidian's `.is-tablet` injects `padding:4px 20px` + bg on EVERY `<button>`, so each custom button must be in the reset `:is()` list or it breaks. Measured on a forced `.is-tablet`: `.windrose-tb-listrow` was getting `4px 20px` (broken). Added this session's new buttons to the resets + re-apply overrides: `.windrose-tb-listrow`/`.windrose-tb-packchip`/`.windrose-tb-packclear` (`_tile-browser.scss`) and `.windrose-dock-floor-del` (`_dock.scss`), plus a `.windrose-dock-floor-row` tap-target bump (`11px 10px` → 47px tall). View-toggle (`.windrose-tb-iconbtn`) + tag chips (`.windrose-tb-chip`) were already covered. Verified live: listrow `0 6px`, floor-row 47px, floor-del 24px neutralized; packchip/packclear rules shipped (`3px 8px` / `0 2px`) and in the reset list (confirmed via live stylesheet — they don't render with the 2 disjoint packs). 0-warning gate held. **All tile-UI punch-list items now complete.**
